...
- when the `list` feature is enabled, `{StructName}List` insertion, lookup, range search, removal, `vacuum`, and sorting, with the same visibility as the record struct
- `compare_all_fields` / `compare_by_fields` / `to_dump_string`

## List Search

`{StructName}List` is an optional helper generated by the default `list` feature. It is useful when you want an in-memory collection with lookup, prefix search, range search, sorting, logical removal, and `vacuum`.
