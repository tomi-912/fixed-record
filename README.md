...
Exact lookup uses the field index directly instead of scanning every record. `find_by` requires exactly the selected field width: shorter input returns `Error::TooShort`, while wider input returns `Error::FieldOverflow`. Prefix, padded-value, range, and sorted lookup use ordered index ranges. `find_range_by` accepts standard Rust ranges whose bounds implement `AsRef<[u8]>`. A short start bound is extended with `0x00`, and a short end bound is extended with `0xFF`, so trailing bytes are unrestricted. Either bound being too wide returns `FieldOverflow`; a start greater than the end returns `InvalidRange`. `push(record)` appends and indexes one record. `insert(index, record)` inserts at a specific position, increments existing indexed IDs at or after that position, and returns `false` when `index > len()`. `update` changes only the affected entries. `remove` unindexes the removed record and decrements later IDs; `sort` and `sort_by` rebuild indexes because the complete order can change. `pop` removes only the last record's index entries because other indexes do not change. This makes repeated searches faster at the cost of additional memory for copied field keys and record indexes.
