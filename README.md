...
`{StructName}List` is an optional helper generated by the default `list` feature. It stores records as `Vec<Box<Record>>` and maintains indexes shaped like `HashMap<Field, BTreeMap<Vec<u8>, Vec<usize>>>`. The outer hash map selects a field without requiring field order. The inner ordered map uses the actual byte sequence stored in that field as its key, and its value contains the current indexes of all matching records. Sorting moves boxes in the vector, not the record values allocated behind them.
