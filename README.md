...
- when the `list` feature is enabled, `{StructName}List` construction from `Vec<Record>`, append, indexed insertion, indexed lookup, index-aware mutation, removal, `pop`, `clear`, and sorting, with the same visibility as the record struct
- `compare_all_fields` / `compare_by_fields` / `to_dump_string`

## List Search and Editing

`{StructName}List` is an optional helper generated by the default `list` feature. It stores records as `Vec<Box<Record>>`. The macro also generates a private `{StructName}ListIndices` type whose fields are typed indexes such as `BTreeMap<Fixed<8>, Vec<usize>>` and `BTreeMap<Fixed<16>, Vec<usize>>`. This private type cannot be named from outside the defining module. Each key is copied directly from its record field without allocating a `Vec<u8>`, and each value contains the current indexes of all matching records. Sorting moves boxes in the vector, not the record values allocated behind them.

Fallible search methods consistently use the `try_` prefix; search methods without it return `Option`, `Vec`, or an iterator directly. Exact lookup uses the field index instead of scanning every record. `try_find_by` requires exactly the selected field width: shorter input returns `Error::TooShort`, while wider input returns `Error::FieldOverflow`. Prefix, padded-value, range, and sorted lookup use ordered index ranges. `try_find_range_by` accepts standard Rust ranges whose bounds implement `AsRef<[u8]>`. A short start bound is extended with `0x00`, and a short end bound is extended with `0xFF`, so trailing bytes are unrestricted. Either bound being too wide returns `FieldOverflow`; a start greater than the end returns `InvalidRange`. `from_records(records)` consumes a `Vec<Record>`, preserves its order, boxes the records, and builds every field index once. The generated `From<Vec<Record>>` implementation delegates to the same construction path, allowing `let list: UserList = records.into()`. `read_from(reader)` consumes a configured `Reader<R, Record>` and collects its remaining records into the indexed List. `push(record)` appends and indexes one record. `insert(index, record)` inserts at a specific position, increments existing indexed IDs at or after that position, and returns `false` when `index > len()`. `update` changes only the affected entries. `remove` unindexes the removed record and decrements later IDs; `sort` and `sort_by` rebuild indexes because record indexes can change. `pop` removes only the last record's index entries because other indexes do not change. `clear` removes all records and clears all indexes. This makes repeated searches faster at the cost of additional memory for copied field keys and record indexes.

`find(predicate)`, `find_all(predicate)`, and `retain(predicate)` are deprecated because they scan every record; `retain` also rebuilds every field index. Prefer the indexed `try_first_by*`, `try_find_by*`, and `try_find_range_by` APIs.

Mutable record references are not exposed through `iter_mut`, because arbitrary changes would leave field indexes stale. `for_each_mut` confines each mutable reference to a callback and rebuilds all indexes afterward. The `try_edit_by*`, `try_edit_range_by`, and `try_edit_first_by*` methods select private current indexes through the same lookup logic as their `try_find*` counterparts, mutate only the matching records, and repair the affected index entries. They return an edited count or a boolean instead of exposing indexes. Drop guards repair indexes if a callback unwinds.
...
`try_find_by` / `try_first_by` require the exact field width. `try_find_by_padded` / `try_first_by_padded` accept shorter input when the remaining field bytes are spaces or `0x00`. Use `first_sorted_by(field)` to retrieve the lowest indexed field value without a `Result`.

Use `try_find_by_prefix` / `try_first_by_prefix` for shorter prefix searches.

Editing equivalents are `try_edit_by`, `try_edit_by_padded`, `try_edit_by_prefix`, `try_edit_range_by`, `try_edit_first_by`, `try_edit_first_by_padded`, and `try_edit_first_by_prefix`. Multiple-record methods return the edited count; first-record methods return whether one record was edited.

## Reader / Writer

`Reader` reads fixed-width records sequentially. `Reader::new` expects LF (`\n`) after records, matching `Writer::new`. If the input uses a different separator or no separator, specify it explicitly. A configured separator is required after every record, including the final record. With the default `list` feature, both `Reader::collect_list()` and `UserList::read_from(reader)` collect all remaining records after applying Reader configuration. Both return the first Reader error without producing a partial List.
