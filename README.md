...
`{StructName}List` is an optional helper generated by the default `list` feature. It stores records as a vector of boxed entries and is useful when you want an in-memory collection with lookup, prefix search, range search, sorting, and physical removal.
