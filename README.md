...
`{StructName}List` is an optional helper generated by the default `list` feature. It stores records as `Vec<Box<Record>>` and maintains an ordered index shaped like `BTreeMap<Field, BTreeMap<Vec<u8>, Vec<usize>>>`. The inner key is the actual byte sequence stored in that field, and its value contains the current indexes of all matching records. Sorting moves boxes in the vector, not the record values allocated behind them.
