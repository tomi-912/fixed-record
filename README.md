...
- when the `list` feature is enabled, `{StructName}List` insertion, lookup, range search, removal, `pop`, and sorting, with the same visibility as the record struct
...
Exact lookup uses the field index directly instead of scanning every record. Prefix, padded-value, range, and sorted lookup use ordered index ranges. `insert` and `update` update the affected index entries; `remove`, `sort`, and `sort_by` rebuild indexes because current vector indexes change. `pop` removes only the last record's index entries because other indexes do not change. This makes repeated searches faster at the cost of additional memory for copied field keys and record indexes.
